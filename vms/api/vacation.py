...
from vms.core.helpers import format_date, verify_dates, get_periods_intersection
...
def vacations_get_list(*args, **kwargs):
    ...
    if "employee_id" in kwargs and len(kwargs['employee_id']) > 1:
        ...
        for i in range(len(vacations) - 1):
            period = get_periods_intersection([[vacations[i].start_date, vacations[i].end_date],
                                               [vacations[i + 1].start_date, vacations[i + 1].end_date]])
            if period and [i for i in period]:
                vacations_intersect.append(period)
        return {'count': len(vacations_intersect), 'results': vacations_intersect}
...
def vacations_post(**kwargs):
    ...
    for vacation in vacations:
        if vacation.start_date > kwargs['end_date'] or kwargs['start_date'] > vacation.end_date:
            ...
            if vacation.start_date == start_date and vacation.end_date == end_date:
                vacation_post = vacation
...
def vacation_delete(id):
    vacation = Vacation.query.get(id)
    result = Vacation.query.filter_by(id=id).delete()
    vacation_schema = VacationSchema()
    data = vacation_schema.dump(vacation)
    db.session.commit()
    return data
